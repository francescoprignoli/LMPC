...
class FTOCP(object):
    ...
    def solve(self, x0, verbose=False, SS=None, Qfun=None, CVX=None):
        ...
        if SS is not None:
            if CVX == True:
                lambVar = cp.Variable(
                    (SS.shape[1], 1), boolean=False
                )  # Initialize vector of variables
            else:
                lambVar = cp.Variable(
                    (SS.shape[1], 1), boolean=True
                )  # Initialize vector of variables
        ...
        if SS is not None:
            constr += [
                SS @ lambVar[:, 0] == x[:, self.N],  # Terminal state \in ConvHull(SS)
                np.ones((1, SS.shape[1])) @ lambVar[:, 0]
                == 1,  # Multiplies \lambda sum to 1
                lambVar >= 0,
            ]  # Multiplier are positive definite
        ...
        if SS is not None:
            cost += (
                Qfun[0, :] @ lambVar[:, 0]
            )  # It terminal cost is given by interpolation using \lambda
        else:
            cost += (
                cp.norm(self.Q**0.5 @ x[:, self.N]) ** 2
            )  # If SS is not given terminal cost is quadratic

        # Solve the Finite Time Optimal Control Problem
        problem = cp.Problem(cp.Minimize(cost), constr)
        if CVX == True:
            problem.solve(
                verbose=verbose, solver=cp.ECOS
            )  # I find that ECOS is better please use it when solving QPs
        else:
