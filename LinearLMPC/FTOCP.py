...
class FTOCP(object):
	...
	def solve(self, x0, verbose = False, SS = None, Qfun = None, CVX = None):
		...
		if SS is not None:
			...
			cost += cp.norm(self.Q**0.5@x[:,self.N])**2 # If SS is not given terminal cost is quadratic


		# Solve the Finite Time Optimal Control Problem
		problem = cp.Problem(cp.Minimize(cost), constr)
		if CVX == True:
			problem.solve(verbose=verbose, solver=cp.ECOS) # I find that ECOS is better please use it when solving QPs
		else:
