...
class LMPC(object):
    ...
    def addTrajectory(self, x, u):
        ...
        print(
            "Performance stored trajectories: \n",
    ...
    def computeCost(self, x, u):
        ...
        for i in range(0, len(x)):
            ...
            if i == 0:
                ...
                cost.append(
                    np.dot(np.dot(x[idx], self.Q), x[idx])
                    + np.dot(np.dot(u[idx], self.R), u[idx])
                    + cost[-1]
                )
    ...
    def solve(self, xt, verbose=False):
        ...
        SS_vector = np.squeeze(
            list(itertools.chain.from_iterable(self.SS))
        ).T  # From a 3D list to a 2D array
        Qfun_vector = np.expand_dims(
            np.array(list(itertools.chain.from_iterable(self.Qfun))), 0
        )  # From a 2D list to a 1D array
