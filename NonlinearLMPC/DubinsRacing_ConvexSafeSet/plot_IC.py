...
xit = []
for i in range(1,it):
	xcl = np.loadtxt('storedData/closedLoopIteration'+str(i)+'_P_'+str(P)+'_IC.txt')
	xcl = xcl.T
...
xcl = np.loadtxt('storedData/closedLoopIteration'+str(it)+'_P_'+str(P)+'_IC.txt')
xcl = xcl.T
iterationTime.append(xcl.shape[1]-1) # Store time to reach xf
xit.append(copy.copy(xcl))
...
for i in range(1,it+1):
	...
	plt.plot(np.array(xclTot)[:,0], np.array(xclTot)[:,1], '-o', color='C0')
plt.plot(np.array(xclTot)[:,0], np.array(xclTot)[:,1], '-o', color='C0', label='LMPC closed-loop trajectories')
...
plt.subplot(2, 1, 1)	
ucl = np.loadtxt('storedData/inputIteration'+str(it)+'_P_'+str(P)+'_IC.txt')
...
plt.figure()
mat = np.loadtxt('storedData/meanTimeLMPC'+'_P_'+str(P)+'_IC.txt')
...
if input == 'y':
	...
	for i in range(0,len(P)):
		ucl = np.loadtxt('storedData/inputIteration'+str(it)+'_P_'+str(P[i])+'_IC.txt')
	...
	for i in range(0, len(P)):
		xcl = np.loadtxt('storedData/closedLoopIteration'+str(it)+'_P_'+str(P[i])+'_IC.txt')
		xclTot = []
		xcl = xcl.T
	...
	plt.figure()
	for i in range(0,len(P)):
		mat = np.loadtxt('storedData/meanTimeLMPC'+'_P_'+str(P[i])+'_IC.txt')
		cost = mat[:,1].tolist()
		cost.insert(0, 100)
