...
class FTOCP(object):
    ...
    def solve(self, x0, xf):
        ...
        self.xGuessTot = np.concatenate(
            (self.xGuess, np.zeros(self.n + self.N - 1)), axis=0
        )
        sol = self.solver(
            lbx=self.lbx,
            ubx=self.ubx,
            lbg=self.lbg_dyanmics,
            ubg=self.ubg_dyanmics,
        ...
        self.uSol = (
            x[(self.N + 1) * self.n : ((self.N + 1) * self.n + self.d * self.N)]
            .reshape((self.N, self.d))
            .T
        )
        self.slack = x[
            ((self.N + 1) * self.n + self.d * self.N) : (
                (self.N + 1) * self.n + self.d * self.N + self.n
            )
        ]
    ...
    def buildNonlinearProgram(self, N, xf):
        ...
        for i in range(0, N):
            constraint = vertcat(
                constraint,
                X[n * (i + 1) + 0]
                - (X[n * i + 0] + X[n * i + 2] * np.cos(U[d * i + 0])),
            )
            constraint = vertcat(
                constraint,
                X[n * (i + 1) + 1]
                - (X[n * i + 1] + X[n * i + 2] * np.sin(U[d * i + 0])),
            )
            constraint = vertcat(
                constraint, X[n * (i + 1) + 2] - (X[n * i + 2] + U[d * i + 1])
            )
        ...
        }  # , "ipopt.acceptable_constr_viol_tol":0.001}#,"ipopt.acceptable_tol":1e-4}#, "expand":True}
    ...
    def f(self, x, u):
        ...
        xNext = np.array(
            [x[0] + x[2] * np.cos(u[0]), x[1] + x[2] * np.sin(u[0]), x[2] + u[1]]
        )
        return xNext.tolist()
