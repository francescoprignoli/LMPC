...
class LMPC(object):
    ...
    def __init__(self, ftocp, l, P, safeSetOption=[]):
        ...
        self.it = 0
        self.safeSetOption = (
            safeSetOption  # Tima varying safe set described in new Nonlinear LMPC paper
        )
        self.itCost = []
        self.N = ftocp.N
    ...
    def addTrajectory(self, x, u):
        ...
        self.xGuess = np.concatenate(
            (
                x[:, 0 : (self.ftocp.N + 1)].T.flatten(),
                u[:, 0 : (self.ftocp.N)].T.flatten(),
            ),
            axis=0,
        )
        ...
        print(
            "Total time old trajectories: ",
            [self.Qfun[x][0] for x in range(0, self.it + 1)],
        )
        ...
        for i in range(0, self.it):
            ...
            if self.P == "all":
                ...
                self.SSindices.append(
                    np.arange(
                        Tj - Tstar + self.ftocp.N, Tj - Tstar + self.ftocp.N + self.P
                    )
                )
    ...
    def solve(self, xt, verbose=5):
        ...
        for i in range(minIt, self.it):
            ...
            for j in idxToTry:
                ...
                if self.ftocp.N > 1:  # if the horizon of the ftocp > 1 ===> solve ftocp
                    if (
                        Qf + N < self.oldCost
                    ):  # check if the cost of the ftocp could be decreasing, otherwise pointless to solve (look LMPC TAC paper the practical implementation section)
        ...
        xflatOpenLoop = self.xOpenLoop[
            :, 0 : (self.ftocp.N + 1)
        ].T.flatten()  # change format
        ...
        if (indList[bestItLocSS][bestTime] + 1) <= (self.SS[bestIt].shape[1] - 1):
            ...
            self.xGuess[n * (N + 1) : (n * (N + 1) + d * (N - 1))] = uflatOpenLoop[
                d : d * N
            ]
            self.xGuess[(n * (N + 1) + d * (N - 1)) : (n * (N + 1) + d * N)] = self.uSS[
                bestIt
