...
def main():
    ...
    safeSetOption = (
        "timeVarying"  # Allowed options are 'timeVarying', 'spaceVarying' and 'all'
    )
    ...
    while itCounter <= itMax:
        ...
        np.savetxt(
            "storedData/closedLoopIteration"
            + str(itCounter)
            + "_P_"
            + str(lmpc.P)
            + ".txt",
            np.round(np.array(xcl), decimals=5).T,
            fmt="%f",
        )
        np.savetxt(
            "storedData/inputIteration" + str(itCounter) + "_P_" + str(lmpc.P) + ".txt",
            np.round(np.array(ucl), decimals=5).T,
            fmt="%f",
        )
        np.savetxt(
            "storedData/meanTimeLMPC_P_" + str(lmpc.P) + ".txt",
            np.array(meanTimeCostLMPC),
            fmt="%f",
        )
        np.save(outfile, xcl)
