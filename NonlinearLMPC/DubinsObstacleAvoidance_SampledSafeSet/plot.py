...
for i in range(1, it):
    xcl = np.loadtxt(
        "storedData/closedLoopIteration" + str(i) + "_P_" + str(P) + ".txt"
    )
    xcl = xcl.T
...
plt.plot(
    ...
    "-o",
    color=colorMap[0],
    label="LMPC closed-loop at " + str(it) + "th iteration",
)
...
if input == "y":
    ...
    for i in range(0, len(P)):
        ucl = np.loadtxt(
            "storedData/inputIteration" + str(it) + "_P_" + str(P[i]) + ".txt"
        )
        if P[i] == "all":
            ...
            plt.plot(
                ucl[:, 0],
                "-o",
                label="LMPC closed-loop for P = " + str(P[i]) + ", i=" + str(l[i]),
            )
    ...
    for i in range(0, len(P)):
        ...
        plt.plot(
            ucl[:, 1], "-o"
        )  # , label="LMPC closed-loop for P = "+str(i)+", i="+str(l[counter]))
...
if input == "y":
    ...
    for i in range(0, len(P)):
        xcl = np.loadtxt(
            "storedData/closedLoopIteration" + str(it) + "_P_" + str(P[i]) + ".txt"
        )
        if P[i] == "all":
    ...
    for i in range(0, len(P)):
        ...
        plt.plot(
            ucl[:, 1],
            "-o",
            label="LMPC closed-loop for P = " + str(i) + ", i=" + str(l[i]),
        )
